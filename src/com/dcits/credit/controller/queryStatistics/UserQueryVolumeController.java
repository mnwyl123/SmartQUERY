 ...
 */
@Controller
@RequestMapping(value="/UserQueryVolume")
public class UserQueryVolumeController extends BaseController {
	
	String menuUrl = "UserQueryVolume/list.do"; //菜单地址(权限用)
	
	@Resource(name="UserQueryVolumeService")
	private UserQueryVolumeManager UserQueryVolumeMapper;
	...
	public ModelAndView list(Page page) throws Exception{
		...
		List<PageData>	varList = UserQueryVolumeMapper.list(page);	//
		mv.setViewName("credit/queryStatistic/userQueryVolume_list");
	...
	public ModelAndView goEdit(Page page)throws Exception{
		...
		page.setPd(pd);
		List<PageData> varList =UserQueryVolumeMapper.detailList(page);
		if(varList.size()==0){
			varList =UserQueryVolumeMapper.detailListHis(page);
		}
		mv.setViewName("credit/queryStatistic/userQueryVolumeDetail_list");
		mv.addObject("varList", varList);
		mv.addObject("pd", pd);
		return mv;
	}	
