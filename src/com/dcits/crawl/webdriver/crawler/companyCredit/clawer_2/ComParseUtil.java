...
//		List<String> xghkzrhz_db = null;
	...
	public static void ratingInfoUtil(Element pjxx_ele,String report_id) throws Exception{
		...
		//截取评级信息的单元代码
		String html_sql = element.html().substring(element_start,element_end);//此处为评级信息到最后一个table的string	
		//当评级信息不是作为最后一个单元的时候
		if(html_sql.contains("m-repbody")){
			html_sql=html_sql.substring(0, html_sql.indexOf("m-repbody"));
		}
		...
		for(int i=0; i<tab_div.size(); i++){
			...
			if(ths.size()==2 && ths.text().indexOf("贴现")>-1){
				if(i<tab_div.size()-1){
					...
					//未结清信贷
					if(trs.select("th").text().indexOf("逾期总额")>-1 && trs.get(0).select("th").size()==7){
						String sql = null;
						for(int j=1; j<trs.size(); j++){
					...
					//已结清信贷
					if(trs.select("th").text().indexOf("逾期总额")==-1){
						String sql = null;
						for(int j=1; j<trs.size(); j++){
							Elements tx_tds = trs.get(j).select("td");
			...
			if(ths.size()==2 && ths.text().indexOf("银行承兑汇票和信用证")>-1){
				if(i<tab_div.size()-1){
					Elements trs = tab_div.get(i+1).select("table tr");
					//未结清信贷
					if(trs.select("th").text().indexOf("垫款标志")==-1){
						for(int j=2; j<trs.size(); j++){
							Elements tx_tds = trs.get(j).select("td");
							String sql = "insert into COM_GUARANTEE_DET(report_id,CODE_ORG,GUAR_TRANS_SPECIES_NICHE,FIVE_CLA,NUM_OUTSTAND_ACCOUNT,IN_THIRTY_DAY_BAL_NUM,IN_SIXTY_DAY_BAL_NUM,IN_NINETY_DAY_BAL_NUM,AFTER_NINETY_DAY_BAL_NUM) values ("
								+ "'"+report_id+"','"+tx_tds.get(0).text()+"','"+tx_tds.get(1).text()+"','"+tx_tds.get(2).text()+"','"+tx_tds.get(3).text()+"','"+tx_tds.get(4).text()+"','"+tx_tds.get(5).text()+"','"+tx_tds.get(6).text()+"','"+tx_tds.get(7).text()+"')";
							loadDetail_sql.add(sql);	 
						}
						i++;
						continue;
			...
			if(ths.size()==2 && ths.text().indexOf("除贴现外的其他业务")>-1){
				if(i<tab_div.size()-1){
					Elements tbodys = tab_div.get(i+1).select("tbody");
					if(tbodys.get(0).select("th").size() == 18){
						for(int j=1; j<tbodys.size(); j++){
							Elements tx_r1_tds = tbodys.get(j).select("tr").get(0).select("td");
							Elements tx_r2_tds = tbodys.get(j).select("tr").get(1).select("td");
							String sql = "insert into COM_BOR_UNDISCOUNT(report_id,CRED_ID,TYPE_REPAY_PES,NUM_GUAR_CONTRA,CURRENCY_REPAY,REPAY_RESP_BALA,CODE_ORG,TYPE_BUSINESS,DATE_OPEN,DUE_TIME_DATE,CURRENCY,"
										+"LEN_SUM,BALANCE_NUM,FIVE_CLA,OVER_SUM,OVER_PRINCIPAL,OVER_MONTH,NUM_MONTH_REMAIN,DATE_INFO_REPORT"
			...
			if(ths.size()==2 && ths.text().indexOf("贴现")>-1){
				if(i<tab_div.size()-1){
					Elements trs = tab_div.get(i+1).select("table tr");
					if(trs.select("th").text().indexOf("责任类型")>-1){
			...
			if(!errDesc.toString().equals(",")){
				System.out.println(errDesc.toString().substring(1)+"模块解析错误，请查看");
...
//		printList(loadDetail_sql);
	...
	//声明及异议标注信息
	public static void DeclareAndDissent(Element smjyybzxx_ele,String report_id) throws Exception{
		Elements title1 = smjyybzxx_ele.select("img");
		Elements table = smjyybzxx_ele.select("table");
		Elements title = smjyybzxx_ele.getElementsByClass("g-tt-h3");//小单元
		int count = 0;			
		for(int i=0;i<title.size();i++){
			String text = title.get(i).text();
			int element_start=smjyybzxx_ele.html().indexOf(title.get(i).html());
			int element_end=0;
			if(i==title.size()-1){
				element_end=smjyybzxx_ele.html().lastIndexOf(">");
			}else{
				element_end=smjyybzxx_ele.html().indexOf(title.get(i+1).html());
			}
			
			if(text==null) continue;
			//民事判决记录
			if(text!=null&&text.contains("征信中心说明")){
				//截取民事判决记录单元下的所有代码
				String html_sql = smjyybzxx_ele.html().substring(element_start,element_end);
				//单元数据解析并入库
				ths_tds_(html_sql,report_id,"COM_CREDIT_DECLARE");
				count++;
			}else if(text!=null&&text.contains("异议标注")){
				//截取强制执行记录单元下的所有代码
				String html_sql = smjyybzxx_ele.html().substring(element_start,element_end);
				//单元数据解析并入库
				ths_tds_(html_sql,report_id,"COM_CREDIT_DISSENT");
				count++;
			}
			if(count>2) 
				break;			
	...
	public static void ths_tds_(String table_html,String report_id,String table_name){
		Map<String,String> htmlMap = new HashMap<String,String>();
		Document document = Jsoup.parse(table_html.toString());
		Elements mspjjl_ele = document.getElementsByTag("TABLE");
		for(Element ele:mspjjl_ele){
			...
			if(ths.size()<tds.size()){//list
				for(int j=0;j<tds.size()/ths.size();j++){
					...
					for(int i=0;i<ths.size();i++){
						...
						htmlMap.put(ths.get(i).text().trim(), tds.get(2*j+i).text().trim());
					}
					//数据落库
					String sql;
					try {
