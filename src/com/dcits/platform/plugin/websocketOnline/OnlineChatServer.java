package com.dcits.platform.plugin.websocketOnline;
...
public class OnlineChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		}else if(null != message && message.startsWith("[onlinemanager]")){
...
//			if(null == OnlineChatServerPool.getWebSocketByIP(str[1])){		//判断用户是否在其它终端登录
				goOut(user);
				OnlineChatServerPool.addUser(user,conn);					//向连接池添加当前的连接对象
				addUserToFhadmin(user,conn);
//			}else{
//				goOut(conn,"goOut");
//			}
		}else{
	 ...
	 * @param conn
	 */
	public void addUserToFhadmin(String user,WebSocket conn){
		//WebSocket conn =  OnlineChatServerPool.getFhadmin();
		if(null == conn){return;}
		JSONObject result = new JSONObject();
