package com.dcits.platform.dao.redis.impl;
...
import com.dcits.platform.dao.AbstractBaseRedisDao;
...
public class RedisDaoImpl extends AbstractBaseRedisDao<String, PageData> implements RedisDao{
	...
	public List<Map<String,String>> getListMap(String key){
		...
		Iterator<String> iter=jedis.hkeys(key).iterator();
		 while (iter.hasNext()){  
	    	 ...
	    	 String ikey = iter.next();  
	    	 	map.put("key", ikey);
	    	 	map.put("value", jedis.hmget(key,ikey).get(0));
	    	 	listMap.add(map);
	    	 }
		 jedis.close();
		 return listMap;
	...
	public Properties getPprVue(){
		InputStream inputStream = DbBackUtil.class.getClassLoader().getResourceAsStream("redis.properties");
