...
<script type="text/javascript" src="<%=basePath%>js/jquery.easytabs.min.js"></script>
<script type="text/javascript" src="<%=basePath%>js/jquery.hashchange.min.js"></script>
...
*/	
	...
	$("#writeBackToDbButton").click(function(){
		collectParams(params);
		params.op = "writeback";
		var args = {};
		args.params= params;
...
*/
	...
	$("select.paramInput").each(function(){
		...
		if(varCap != null && $.trim(varCap)!="") {
			setParameter(varCap, $(this).selectedTexts()[0] ? $(this).selectedTexts()[0].replace(/\u00a0+/,"") : '');
		}
		if(rmlpar!="") {
			rmlpar += "|";
		}
	...
	$("input.paramInput[type=text]").each(function(){
	    if($(this).hasClass("itext")){
	    	...
	    	setParameter($(this).attr("varCap"), cap);
	    	if(cap == ""){
		...
		if($(this).hasClass("datePicker")){
			setParameter($(this).attr("varCap"), $.datepicker.formatDate($(this).attr("capFormat"),$(this).datepicker("getDate")));
		}
	});
			...
			if(yearWrap.selectedValues()[0] == "") {
				...
				switch($(this).attr("optionType").toLowerCase()){
					...
					case "ymt":{
						...
						setParameter($(this).attr("varCap"), yearWrap.selectedTexts()[0] + mWrap.selectedTexts()[0] + dWrap.selectedTexts()[0]);
						break;
					}
					case "ym":
					case "ys":
					...
					case "yw": {
						...
						setParameter($(this).attr("varCap"), yearWrap.selectedTexts()[0] + mWrap.selectedTexts()[0]);
						break;
					}
					case "y":{
						params[$(this).attr("var")] = yearWrap.selectedValues()[0];
						setParameter($(this).attr("varCap"), yearWrap.selectedTexts()[0]);
						break;
					}
				}
...
function displayAsTab(data){
	
	var div_height = $("#centerPan").height() - 40;
	
	$("#centerPan").css("text-align", "left");
	
	$("#centerPan").html("<div id='tab-container' class='tab-container'> <div id='panel-container' class='panel-container'><div style='height:"+div_height+"px; overflow:auto' id='home_page'></div></div><ul id='ui_tab' class='etabs'><li ><a href='#home_page'>首页</a></li></ul></div>");
	$("#home_page").css("padding-top",$("#toolbar").height());
	
	$("#home_page").html(data.DataInfo.content);
	
	$("#ui_tab").after("<div id='movetab_div' class='div-move-tab'><input id='bt_moveLeft' type='button' disabled='disabled' value='<' onclick='tabMoveLeft()'/>&nbsp;<input id='bt_moveRight' disabled='disabled' type='button' value='>' onclick='tabMoveRight()'/></div>")
	
	var total_width = $("#movetab_div").outerWidth() + 10;
	
	total_width += ($("#ui_tab li").eq(0).outerWidth() + 5);
	
	$(".regularTable tr").each(function(rowNo){
		if(rowNo > 0){
			var text = $(this).children().eq(0).text();
			$("#panel-container").append("<div style='height: "+div_height+"px; overflow:auto' id='pc_"+rowNo+"'>2</div>");
			$("#ui_tab").append("<li><a id='tab_a_"+rowNo+"' href='#pc_"+rowNo+"'>"+text+"</a></li>");
			var tab_width = $("#ui_tab li").eq(rowNo).outerWidth() + 5;
			total_width += tab_width;
		//	alert($("#ui_tab li").eq(rowNo).outerWidth()+" : "+total_width+":  "+$("#tab-container").width())
			if(total_width > $("#tab-container").width()){
				$("#ui_tab li").eq(rowNo).css("display", "none");
				$('#bt_moveRight').removeAttr("disabled");
			}
		}
	});
	
	$('#tab-container').easytabs();
	$('#tab-container').bind("easytabs:before", changeTabBefore);
	
	//解决IE7/8的UI兼容问题
	if(isIE7() || isIE8() || isIE9()){
		$("#ui_tab li").each(function(rowNo){
    		$("#ui_tab li").eq(rowNo).css("padding-bottom","0");
    	});
	}
}
...
var block = isIE7()||isIE8()||isIE9()?"inline":"inline-block";

function changeTabBefore(e, c, t, s){
	...
	if(divId.indexOf("pc_")>-1){
		if($(t).html().length > 1)
			return true;
		var rowNum = divId.substring(3, divId.length);
		var tag = $(".regularTable .op[rowno='"+(rowNum-1)+"'][op='view']");
	}else
		return true;
	
	var inputParams = parseUrlQuery($(tag).attr('param'));
		collectParams(params);
	var newParams = {};
	...
	$(t).html("<iframe frameborder=0 class='iframe-tab' src='"+href+"'/>");
	return true;
}
...
function tabMoveRight(){
	...
	$("#ui_tab li").each(function(rowNo){
		if($("#ui_tab li").eq(rowNo).css("display") == block){
			leftTabNo = rowNo;
			return false;
		}
	});
	
	for(var rowNo=$("#ui_tab li").length-1; rowNo>=leftTabNo; rowNo--){
		if($("#ui_tab li").eq(rowNo).css("display") == block){
			if(rowNo + 1 == $("#ui_tab li").length)
				return false;
			if(!hasDisplay){
				$("#ui_tab li").eq(rowNo+1).css("display", block);
				total_width += ($("#ui_tab li").eq(rowNo+1).outerWidth() + 5);
				hasDisplay = true;
			}
			total_width += ($("#ui_tab li").eq(rowNo).outerWidth() + 5);
			if(total_width > $("#tab-container").width()){
				$("#ui_tab li").eq(rowNo).css("display", "none");
			}
		}
...
function tabMoveLeft(){
	...
	for(var rowNo=$("#ui_tab li").length-1; rowNo>=0; rowNo--){
		if($("#ui_tab li").eq(rowNo).css("display") == block){
			rightTabNo = rowNo;
			break;
		}
	...
	for(var rowNo=0; rowNo<=rightTabNo; rowNo++){
		if($("#ui_tab li").eq(rowNo).css("display") == block){
			if(rowNo == 0)
				return false;
			if(!hasDisplay){
...
function hideOrShowMoveButton(){
	if($("#ui_tab li").eq(0).css("display") == "none"){
		$('#bt_moveLeft').removeAttr("disabled");
	}else{
		$('#bt_moveLeft').attr("disabled", "disabled");
	}
	
	if($("#ui_tab li").eq($("#ui_tab li").length-1).css("display") == "none"){
		$('#bt_moveRight').removeAttr("disabled");
	}else{
		$('#bt_moveRight').attr("disabled", "disabled");
	}
}
...
function bindClick2Tr() {
	$(".regularTable tr").each(function(rowNo){
	    ...
	    */
	    	...
	    	if($($(this).children()[3]).css("display") != "none") {
	        ...
	        var href = "";
			$(this).children().each(function(colNo){
			...
			$($(this).children()[0]).addClass("op").attr("op","view").attr("rowNo",(rowNo-1)).attr("param",href);
			if($($(this).children()[3]).css("display") != "none") {
				$(this).append("<td><span class='op' op='compare' style='margin-left:0.5em;display:none;' rowNo='" + (rowNo-1) +"' param='" + href + "'>检查</span>"
	...
	$(".regularTable .op").click(function(){
		if(isDisplayAsTab && $(this).attr('op') == "view"){
			var aid = "tab_a_"+(parseInt($(this).attr('rowno'))+1);
			$("#"+aid).click();
			return;
		}
	 	if($(this).attr('op') == "view"){
			...
			*/
	 		...
	 		$.each(invalidateMsgs[idx].aliasIdxMap,function(key,val){
	 			var ctrl = $(".regularTable tr td[rowNo="+val+"]");
	 		...
	 		showDialog(url,"数据比较",screen.availWidth,screen.availHeight,dialogParam)
	 	}
	});
}
...
#tab-container ul { float: left;margin: 0; padding: 0; padding-left: 10}
#tab-container ul li { margin-right:5px; display: inline-block; zoom:1; *display:inline; background: #eee; border: solid 1px #999; border-top: none; -moz-border-radius: 0 0 4px 4px; -webkit-border-radius: 0 0 4px 4px; }
#tab-container ul li a { font-size: 12px; line-height: 2em; display: block; padding: 0 8px; outline: none; }
#tab-container ul li a:hover { text-decoration: underline; }
#tab-container ul li.active { background: #fff; padding-bottom: 6px; position: relative; bottom: 1px; border-color: #666; }
#tab-container ul li a.active { font-weight: bold; text-decoration: none; }
#tab-container .panel-container { border-bottom: solid #666 1px; padding: 0px; margin-top: 0; }
.div-move-tab { float: right; display: inline; padding-right: 10}
.iframe-tab {width: 100%; height:100%; border:0}
...
<body id="${bodyId}"
	...
	<div id="centerPan" class="ui-layout-center"
		style="position: relative;z-index:1; top: 0;margin: 0; text-align: center; width: 100%; height: 100%; overflow: auto;">
		${reportHtml }
