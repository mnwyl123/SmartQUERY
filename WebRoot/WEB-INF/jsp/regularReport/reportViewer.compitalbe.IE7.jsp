...
<link rel="stylesheet" type="text/css" href="<%=basePath%>css/zTreeStyle.3.5/zTreeStyle.css" >
<link rel="stylesheet" type="text/css"
...
function timerCallback() {
	var $container = $("#formPanel");//$(window).height() > 100 ? $(window) : $("body");	
...
//		alert(maxlen);
	...
	*/
	...
	$("#accessCtrlBtn").click(function(){
		...
		//open(page);
		showModalDialog( "<%=basePath%>" + page, "",  "dialogWidth:600px;dialogHeight:450px;resizable:yes;center:yes;");
	});
...
function drawTopLeftCell() {
	...
	$(".topLeft2bottomRightLine").each(function(){
		painter.drawLine($(this).offset().left,$(this).offset().top - $(".toolbar").height(),
			$(this).offset().left+$(this).width(),
			$(this).offset().top + $(this).outerHeight() - $(".toolbar").height());
		/*
		...
		$("table.regularTable td").each(function() {
			...
			if(tmpCells[id]){
				...
				$(this).attr("valign","bottom")
					...
					.html("<input type='text' id='" + val + "' idx='" + idx + "'style='border:none;border-bottom:solid 1px red;width:auto;' value='" + ((draftVal!=null) ? draftVal : $(this).attr("val"))  + "' />");
			}   
		});
/*		
...
function initCalendarControl(){
	...
	$("#ui-datepicker-div").appendTo($("#formPanel"));
		...
		*/
		...
		if(data.DataInfo.includeAuditData) {
			...
			$("img.jTip").hover(function(){
					$("#JT").remove();
					JT_showHtml(this.id,genHistoryInputsMsg($(this).attr("cooStr")),null,null,$("#formPanel"));
			});
		}
...
//alert(expr);				
					...
					$("img.jTip").hover(function(){
						$("#JT").remove();
						JT_showHtml(this.id,genIvalidateMsg($(this).attr("cooStr"),$(this).attr("expr"),$(this).attr("exprIdx")),null,null,$("#formPanel"));
					});
				} else {
					alert(data.HeadInfo.Message);
				}
...
<style type="text/css">
	...
	ul.ztree {margin-top: 10px;border: 1px solid #617775;background: white;width:220px;height:360px;overflow-y:scroll;overflow-x:auto;}
	a.treeOp {
		margin-left:0.5em;
		margin-right:0.5em;
		float:right;
	...
	div#firstTimeComments {
		...
		background: url(css/base/images/blankreport/background.jpg) no-repeat 20% top ;
...
-->
	...
	<div id="centerPan" class="ui-layout-center"
		style="position: relative; top:36px; padding: 1em; margin: 0; text-align: center; width:100%;height:100%; overflow: auto;">
		${reportHtml }
	...
	<div id="southPan" class="statusbar">
		...
		<div id="pageNavi" style="display:none;float:right;">
			...
			<a id='gotoPageBtn' href='#'>转到</a><input id='gotoPage'  type='text' maxlength='5' />页
		</div>
	</div>
	...
	<div id="editPanel" class="southPanel" style="display:none;position:absolute;bottom:0;left:0; width:100%;z-index:200;background-color:gray;">
		<div style='margin-top:.5em;margin-left:.5em;'>
		录入补录原因:
		</div>
		<textarea id='inputComments' style='width:100%;height:5em;'></textarea>
		<div style="float:right;width:100%;">
		<input id='inputCancel' type='button' value='取消' style='float:right;margin-left:1em;margin-right:1em;' />
		<input id='inputSave' type='button' value='存为草稿' style='float:right;margin-left:1em;margin-right:1em;' />
		<input id='inputOK' type='button' value='提交' style='float:right;margin-left:1em;margin-right:1em;' />"
		</div>
	</div>
	
	<div id = "auditEditPanel" class="southPanel" style="display:none;position:absolute;bottom:0;left:0; width:100%;z-index:200;background-color:gray;">
