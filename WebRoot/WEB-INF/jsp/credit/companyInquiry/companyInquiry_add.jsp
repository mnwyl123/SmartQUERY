		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					...
					<form action="CompanyInquiry/${msg}.do?INTERNAL=2" name="Form" id="Form" method="post" enctype="multipart/form-data">
						...
						<table id="table_report" class="table table-striped "><!-- table-bordered table-hover -->
							...
							<tr>
								...
								<td><input type="text" name="COMPANY_NAME" id="COMPANY_NAME" value="${pd.COMPANY_NAME}" maxlength="255" placeholder="这里输入客户名称" title="客户名称" class="width-75"/></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td><input type="text" name="CHINA_CREDIT_CODE" id="CHINA_CREDIT_CODE" value="${pd.CHINA_CREDIT_CODE}" maxlength="255" placeholder="这里输入企业证件号码" title="企业证件号码" class="width-75"/></td>
							</tr>
							<tr>
							...
							<tr>
							    <td class="align-right width-25">贷款申请书上传:</span></td>
								<td class="z_photo upimg-div clear">
									...
									<section class="z_file fl">
									    <c:if test="${pd == null || pd.LOAN_APPLICATION == '' || pd.LOAN_APPLICATION == null }">
										 ...
										 <c:if test="${pd != null && pd.LOAN_APPLICATION != '' && pd.LOAN_APPLICATION != null }">
												...
												<c:if test="${pd.LOAN_APPLICATIONTYPE == 'pdf'}">
													<input  value="${pd.LOAN_APPLICATIONTYPE}" />
												</c:if>
										...
										<input type="file" name="LOAN_APPLICATION" id="LOAN_APPLICATION" value="${pd.LOAN_APPLICATION}" style="width:80%;" class="file" accept="image/jpg,image/jpeg,image/png,image/bmp,application/pdf"/>
									 </section>
								</td>
								<aside class="mask works-mask">
									<div class="mask-content">
									...
									<section class="z_file fl">
									    <c:if test="${pd == null || pd.IDENTITY_DOCUMENT == '' || pd.IDENTITY_DOCUMENT == null }">
										 ...
										 <c:if test="${pd.IDENTITY_DOCUMENTTYPE != 'pdf'}">
												...
												<c:if test="${pd.IDENTITY_DOCUMENTTYPE == 'pdf'}">
													<input  value="${pd.IDENTITY_DOCUMENTTYPE}" />
												</c:if>
												<a href="${pd.IDENTITY_DOCUMENTTYPE}" target="_blank"><img src="img/a8.png"/></a>
										 </c:if>
...
<!-- /.main-container -->
	...
	<script type="text/javascript">
		...
		function save(){
			...
			if($("#CHINA_CREDIT_CODE").val()==""){
				$("#CHINA_CREDIT_CODE").tips({
					side:3,
		            msg:'请输入企业证件号码',
		            bg:'#AE81FF',
		            time:2
		        });
				$("#CHINA_CREDIT_CODE").focus();
			return false;
			}
			if($("#CHINA_CREDIT_CODE").val()!=""&&$("#CHINA_CREDIT_CODE").val()!=null){
				if(($("#CREDENTIALS_NO_TYPE").val()=="12")||($("#CREDENTIALS_NO_TYPE").val()=="30")){
					var CHINA_CREDIT_CODE=$("#CHINA_CREDIT_CODE").val();
					if($("#CHINA_CREDIT_CODE").val().length!=10||CHINA_CREDIT_CODE.substr(8,1)!="-"){
						$("#CHINA_CREDIT_CODE").tips({
							side:3,
			...
			if($("#CHINA_CREDIT_CODE").val()!=""&&$("#CHINA_CREDIT_CODE").val()!=null){
				if(($("#CREDENTIALS_NO_TYPE").val()=="11")||($("#CREDENTIALS_NO_TYPE").val()=="10")){
					if($("#CHINA_CREDIT_CODE").val().length!=16){
						$("#CHINA_CREDIT_CODE").tips({
							side:3,
	...
	//		if($("#CHINA_CREDIT_CODE").val()!=""&&$("#CHINA_CREDIT_CODE").val()!=null){
	//			if($("#CREDENTIALS_NO_TYPE").val()=="10"){
	//				if($("#CHINA_CREDIT_CODE").val().length!=18){
	//					$("#CHINA_CREDIT_CODE").tips({
	//						side:3,
	...
	//		}
			if($("#CHINA_CREDIT_CODE").val()!=""&&$("#CHINA_CREDIT_CODE").val()!=null){
				if(($("#CREDENTIALS_NO_TYPE").val()=="12")||($("#CREDENTIALS_NO_TYPE").val()=="30")){
					if($("#CHINA_CREDIT_CODE").val().length!=10){
						$("#CHINA_CREDIT_CODE").tips({
							side:3,
