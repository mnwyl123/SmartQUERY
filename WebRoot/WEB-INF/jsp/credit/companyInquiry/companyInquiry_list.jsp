			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="CompanyInquiry/list.do?menuId=30" method="post" name="Form" id="Form">
			            <table class="margin-5 width-100">
							<tr>
								<td class="align-left width-70">
									...
									<div class="float-l margin2-5">	
										<input id="CHINA_CREDIT_CODE" type="text" name="CHINA_CREDIT_CODE" value="${pd.CHINA_CREDIT_CODE}" value="" placeholder="企业证件号码"/>
									</div>
											...
											<td class="center">
												...
												<c:if test="${var.STATUS=='00'|| var.STATUS=='03'}">
													...
													<c:if test="${QX.delete == 1 }">
														<i class="iconfont icon-shape7 green2" title="删除"  onclick="deleteApply('${var.ID}');"></i>
													</c:if>
	...
	<script type="text/javascript">
		...
		function add(){
			if(${pd.ORG_ID==""} ){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>CompanyInquiry/goAdd.do';
			 diag.Width = 510;
			 diag.Height =600;
		...
		function edit(ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改";
			 diag.URL = '<%=basePath%>CompanyInquiry/goRevise.do?ID='+ID;
			 diag.Width = 510;
			 diag.Height = 640;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		/*function deleteU(ID){
			...
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					alert(ID+"&&&&&&");
					top.jzts();
					var url = "<%=basePath%>CompanyInquiry/delete.do?ID="+ID;
		...
		function submitApply(ID){
			bootbox.confirm("提交之后不可更改，确定要提交吗?", function(result) {
				if(result) {
					...
					$.ajax({
						type: "POST",
						url: '<%=basePath%>CompanyInquiry/goSubmit.do?ID='+ID,
						data: {DATA_IDS:""},
						dataType:'json',
		...
		function AgainApply(ID){
			...
			$.ajax({
				type: "POST",
				url: '<%=basePath%>CompanyInquiry/goApplyAgain.do?ID='+ID,
				data: {DATA_IDS:""},
				dataType:'json',
				//beforeSend: validateData,
				cache: false,
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要提交选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>CompanyInquiry/goSubmitBatch.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
								//beforeSend: validateData,
								cache: false,
