						...
						<table id="table_report" class="table table-striped table-bordered table-hover" style="width:100%;table-layout:fixed">
							<thead>
								<tr>
									...
									<th class="center">企业查询失败笔数</th>
									<th class="center">企业查询失败比例(%)</th>
									<th class="center">个人查询笔数</th>
									<th class="center">个人查询失败笔数</th>
									<th class="center">个人查询失败比例(%)</th>
								</tr>
							</thead>					
							<tbody>
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty failPercentToLargelist}">
									<c:forEach items="${failPercentToLargelist}" var="dsl" varStatus="vs">											
										<tr>
											...
											<td class="center">${dsl.ORGQUERYFAILCNT}</td>
											<td class="center">${dsl.ORGFAILPERCENT}</td>
											<td class="center">${dsl.PERSONQUERYCNT}</td>
											<td class="center">${dsl.PERSONQUERYFAILCNT}</td>
											<td class="center">${dsl.PERSONFAILPERCENT}</td>
										</tr>
...
<script type="text/javascript">
	...
	$(function() {
			...
			//添加合计
			var varList = '${failPercentToLargelist}';
			if(varList!='[]'){
			    ...
			    var totalFailCom = 0;
			    var totalFailPer = 0;
			    <c:forEach items="${failPercentToLargelist}" var="var" varStatus="vs">
			        totalCom += parseInt('${var.ORGQUERYCNT}');
			        totalPer += parseInt('${var.PERSONQUERYCNT}');
			        totalFailCom += parseInt('${var.ORGQUERYFAILCNT}');
			        totalFailPer += parseInt('${var.PERSONQUERYFAILCNT}');
			    </c:forEach>
			    tr += "<td class='center'>"+totalCom+"</td>";
			    tr += "<td class='center'>"+totalFailCom+"</td>";
			    tr += "<td colspan='1'></td>";
			    tr += "<td class='center'>"+totalPer+"</td>";
			    tr += "<td class='center'>"+totalFailPer+"</td>";			
