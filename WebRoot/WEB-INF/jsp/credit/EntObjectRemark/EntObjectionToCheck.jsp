...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
					<form action="entObjectRemark/list.do?menuId=92" method="post" name="Form" id="Form">
						...
						<table class="" id="tab">
							...
							<tr>
								...
								<td>
											<select style="width:163px" dict-name="clientType" default-value="${pd.ENCLOSUREFLAG}" name="ENCLOSUREFLAG" id="ENCLOSUREFLAG"  />
													<option value="">--</option>
					...
					<form action="entObjectRemark/batchUpdateObjOrg" id="Form2" name="Form2" method="post">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover">	
							<thead>
								<tr>
								    ...
								    <th class="center" style="min-width:100px">企业名称</th>
									<th class="center" style="min-width:100px" dict-name="EntIDType">企业身份标识类型</th>
									<th class="center" style="min-width:100px">企业身份标识号码</th>
									...
									<th class="center" style="min-width:100px" dict-name="Entywzl">业务种类</th>
	...
	<script type="text/javascript">
		...
		function goSub(ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="异议信息";
			 diag.URL = '<%=basePath%>entObjectRemark/listSubToCheck.do?ID='+ID;
			 diag.Width = 800;
			 diag.Height = 600;
					...
					bootbox.dialog({
						...
						buttons: 			
						{
							"button" :
							{
								...
								"callback": function() {
									$("#Form2").attr("action","entObjectRemark/list.do");
									$("#Form2").submit();
								}
							}
						}
		...
		function goSubLast(OBJCHECKID){
			 ...
			 diag.Drag=true;
			 diag.Title ="历史信息";
			 diag.URL = '<%=basePath%>entObjectRemark/listSubLastToCheck.do?OBJCHECKID='+OBJCHECKID;
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function download(OBJITEMINF){	
             top.jzts();
            $.ajax({
            	url:'<%=basePath%>entObjectRemark/download_ITEMINF.do',
            	type:'POST',
            	...
            	success:function(data){
            		...
            		var msg = "";
            		if("success" == (data.msg)){			
            			window.location.href='<%=basePath%>entObjectRemark/downloadlocal_ITEMINF.do?OBJITEMINF='+OBJITEMINF;
            		}else{
		...
		function attrInfo(ID){
			$.ajax({
				url:'<%=basePath%>entObjectRemark/isExists?ID='+ID,
				type:'post',
				dataType:'json',
				success:function(data){
					if(data.msg=='Y'){
						window.location.href='<%=basePath%>entObjectRemark/downloadAttr.do?filepath='+data.filePath+'&fileFTPPath='+data.fileFTPPath;
					}else{
						bootbox.dialog({
