...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="entObjectRemark/listObjAddRemark.do?menuId=97" method="post" name="Form" id="Form">
						<table class="" id="tab">
							<tr>
								<td align="right">
									发起用户名称:
	...
	<script type="text/javascript">
		...
		function add1(listSizeIsNull){
			 ...
			 diag.Drag=true;
			 diag.Title ="处理结果填写";
			 diag.URL = '<%=basePath%>entObjectRemark/goAddPersonLabel.do?';
			 diag.Width = 600;
			 diag.Height = 450;
		...
		function writeResult(ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="处理结果填写";
			 diag.URL = '<%=basePath%>entObjectRemark/goAddPersonLabel.do?ID='+ID;
			 diag.Width = 600;
			 diag.Height = 450;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(ID,SERVICERETURNCODE){
			bootbox.confirm("确定向人行发起删除此人标记的请求吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>entObjectRemark/deleteEntLabel.do?ID="+ID+"&SERVICERETURNCODE="+SERVICERETURNCODE;
