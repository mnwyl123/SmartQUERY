			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="Channel/listChannel.do?menuId=${pd.menuId}" method="post" name="Form" id="Form">
								...
								<c:if test="${QX.query == 1 }">
									<td class="align-left width-70">
										<div class="float-l margin2-5">
											<span>渠道编号:</span>
											<input type="text" style="width:180px" name="CHANNEL_ID" id="CHANNEL_ID" placeholder="请输入渠道编号" />
										</div>
										<div class="float-l margin2-5">
										    <span>&nbsp;渠道描述:</span>
										    <input type="text" style="width:180px" name="CHANNEL_DES" id="CHANNEL_DES" placeholder="请输入渠道描述" />
										</div>
										<div class="float-l margin2-5">
											<span>&nbsp;渠道状态:</span>
											<select style="width:180px" dict-name="channel_status" default-value="${pd.CHANNEL_STATUS}" name="CHANNEL_STATUS" id="CHANNEL_STATUS">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" >
							<thead>
								<tr>
									...
									<th class="center" dict-name="channel_status">渠道状态</th>
								...
								<c:when test="${not empty listInfo}">
									<c:if test="${QX.query == 1 }">
										<c:forEach items="${listInfo}" var="var" varStatus="vs">	
											<tr>
												<td class='center'>
													<label><input type='checkbox' name='ids' id="${var.CHANNEL_ID}" class="ace"/><span class="lbl"></span></label>
												</td>
												<td class="center">${var.CHANNEL_ID }</td>
												<td class="center">${var.CHANNEL_DES }</td>
												...
												<td class="center">
													...
													<div class="hidden-sm hidden-xs action-buttons">
														<c:if test="${QX.edit == 1 }">
														<a  href="javascript:edit('${var.CHANNEL_ID}');">
															<i class="iconfont icon-bianji red1 " title="编辑" ></i>
														</a>
														</c:if>
											  			<c:if test="${QX.delete == 1 }">
											  			<a  href="javascript:del('${var.CHANNEL_ID }');">
													...
													<div class="hidden-md hidden-lg">
														<div class="inline pos-rel">
															...
															<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
																<c:if test="${QX.edit == 1 }">
																<li>
																	<a  onclick="edit('${var.CHANNEL_ID}');" class="tooltip-success" data-rel="tooltip" title="编辑">
																		<span>
																...
																<c:if test="${QX.delete == 1 }">
																<li>
																	<a  onclick="del('${var.CHANNEL_ID }');" class="tooltip-error" data-rel="tooltip" title="删除">
																		<span>
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>Channel/goEdit.do?method=add';
	 diag.Width = 469;
	 diag.Height = 250;
...
function edit(Id){
	 ...
	 diag.Drag=true;
	 diag.Title ="修改";
	 diag.URL = '<%=basePath%>Channel/goEdit.do?method=edit&CHANNEL_ID='+Id;
	 diag.Width = 469;
	 diag.Height = 250;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
