			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="HistoryInquery/listHistoryComRe.do?menuId=${pd.menuId}" method="post" name="Form" id="Form">
						<table class="margin-5 width-100">
							<td class="align-left width-70">
								...
								<div class="float-l margin2-5">	
									<span>&nbsp;证件号类型:&nbsp;<span>
									<select  dict-name="COM_CRED_TYPE" default-value="${pd.CREDENTIALS_NO_TYPE}" name="CREDENTIALS_NO_TYPE" id="CREDENTIALS_NO_TYPE" style="width: 120px;">  
									...
									<c:if test="${QX.query == 1 }">
										<c:forEach items="${varList}" var="var" varStatus="vs">	
											<tr>
												...
												<td class="center">
													<i class="menu-icon fa fa-book icon-shape7se" title="详细信息" onclick="goDetail('${var.ID}','${var.USER_SYS_ID}');" ></i>
...
function goDetail(ID,USER_SYS_ID,CLIENT_TYPE){
	 ...
	 diag.Drag=true;
	 diag.Title ="查询申请历史详情";
	 diag.URL = '<%=basePath%>HistoryInquery/getComHistoryInfo.do?ID='+ID+'&USER_SYS_ID='+USER_SYS_ID;
	 diag.Width = 500;
	 diag.Height = 550;
...
function edit(ID){
	 ...
	 diag.Drag=true;
	 diag.Title ="修改";
//	 diag.URL ='<%=basePath%>HistoryInquery/goAdd.do?ID='+ID;
	 diag.URL = '<%=basePath%>SavePersonInquiryController/goReviseCompany.do?ID='+ID;
	 diag.Width = 510;
	 diag.Height = 640;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
