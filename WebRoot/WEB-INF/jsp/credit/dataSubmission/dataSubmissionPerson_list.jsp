		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					...
					<form action="dataSubmission/dataSubmissionPersonlist.do?menuId=${pd.menuId}" name="Form" id="Form" method="post">
						<div id="zhongxin">
						<table class="margin-5 width-100">
							<tr>
								<td class="align-left">
									...
									<div class="float-l margin2-5">
										<span>证件号码:</span>
										<input type="text"style="width:180px" name="CREDENTIALS_NUM" id="CREDENTIALS_NO" placeholder="请输入证件号码" />
									</div>
									<div class="float-l margin2-5">&nbsp;&nbsp;</div>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
									...
									<th class="center"  style="min-width:100px">客户名称</th>
									<th class="center" style="min-width:170px">查询时间</th>
									<th class="center" dict-name="CRED_TYPE"  style="min-width:100px">证件类型</th>
									<th class="center" style="min-width:180px">证件号码</th>
									<th class="center" dict-name="FIND_REASON" style="min-width:100px">查询原因</th>
									<th class="center" dict-name="FIND_VERSION" style="min-width:100px">查询版本</th>
									<th class="center" style="min-width:50px">是否查得</th>
									<th class="center" style="min-width:170px">查询授权时间</th>
									<th class="center" style="min-width:100px">查询机IP</th>
									<th class="center" style="min-width:50px">操作</th>
								</tr>
							</thead>					
							<tbody>
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty dataSubmissionPersonList}">
									<c:forEach items="${dataSubmissionPersonList}" var="dsl" varStatus="vs">											
										<tr style="height: 45px;">
											...
											<td class="center">${dsl.QUERY_REASON}</td>
											<td class="center">${dsl.QUERY_VERSION}</td>
											...
											<td class="center">
												...
												<i class="iconfont icon-yingshe01 orange1" title="发送报文" onclick="sendPost('${dsl.ID}');" ></i> -->
												<c:if test="${QX.send == 1 }">
													<c:if test="${dsl.SEND_STATUS!='1'}">
...
<script type="text/javascript">
	...
	function createXml(id){
		$.ajax({
			type: "POST",
			url: '<%=basePath%>dataSubmission/createXmlOfPerson.do',
	    	data: {id:id},
			dataType:'json',
			cache: false,
	...
	function createTxt(id){
		$.ajax({
			type: "POST",
			url: '<%=basePath%>dataSubmission/createTxtOfPerson.do',
	    	data: {id:id},
			dataType:'json',
			cache: false,
			success:function(data){
