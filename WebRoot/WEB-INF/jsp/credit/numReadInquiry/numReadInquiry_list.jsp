			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="queryNumRead/list.do?menuId=26" method="post" name="Form" id="Form">
			            <table class="margin-5 width-100">
							<tr>
								<td class="align-left width-70">
									...
									<div class="float-l margin2-5">
									    <span>证件类型:<span>
										<select class="" name="CREDENTIALS_TYPE" dict-name="CRED_TYPE_PER" default-value="${pd.CRED_TYPE}" id="CREDENTIALS_TYPE" data-placeholder="证件类型" style="width: 120px;">
										    <option value="">-请选择-</option>
								  		</select>
									</div>
									<div class="float-l margin2-5">	
										<input id="CREDENTIALS_NUM" type="text" name="CREDENTIALS_NO" value="" placeholder="证件号码"/>
									</div>
	...
	<script type="text/javascript">
		...
		function add(){
			if(${pd.ORG_ID==""} ){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>queryNumRead/goAdd.do';
			 diag.Width = 510;
			 diag.Height = 400;
		...
		function edit(ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改";
			 diag.URL = '<%=basePath%>queryNumRead/goRevise.do?ID='+ID;
			 diag.Width = 510;
			 diag.Height = 400;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function deleteU(ID){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>queryNumRead/delete.do?ID="+ID;
		...
		function submitApply(ID){
			bootbox.confirm("提交之后不可更改，确定要提交吗?", function(result) {
				if(result) {
					...
					$.ajax({
						type: "POST",
						url: '<%=basePath%>queryNumRead/goSubmit.do?ID='+ID,
						data: {DATA_IDS:""},
						dataType:'json',
		...
		function AgainApply(ID){
			...
			$.ajax({
				type: "POST",
				url: '<%=basePath%>queryNumRead/goApplyAgain.do?ID='+ID,
				data: {DATA_IDS:""},
				dataType:'json',
				//beforeSend: validateData,
				cache: false,
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要提交选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>queryNumRead/goSubmitBatch.do?tm='+new Date().getTime(),
								data: {DATA_IDS:str},
								dataType:'json',
								//beforeSend: validateData,
								cache: false,
		...
		function goDelete(ID){
			bootbox.confirm("确定丢弃本条数据?", function(result) {
				if(result) {
					...
					$.ajax({
						type: "POST",
						url: '<%=basePath%>queryNumRead/goDelete.do?ID='+ID+'&STATUS=12',
						data: {DATA_IDS:""},
						dataType:'json',
						//beforeSend: validateData,
						cache: false,
