			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="ServerUtil/listServer.do?menuId=${pd.menuId}" method="post" name="Form" id="Form">
							...
							<tr>
								<c:if test="${QX.query == 1 }">
									<td class="align-left">
										...
										<div class="float-l margin2-5">
											<span>证件号码:</span>
											<input type="text"style="width:180px" name="CREDENTIALS_NUM" id="CREDENTIALS_NUM" placeholder="请输入证件号码" />
										</div>
									...
									<c:if test="${QX.query == 1 }">
										<c:forEach items="${listInfo}" var="var" varStatus="vs">	
											<tr>
												...
												<td class="center">${var.CREDENTIALS_NUM }</td>
												<td class="center">${var.APPLICATION_TIME}</td>
												<td class="center">${var.FIND_REASON}</td>
												<td class="center">${var.FIND_VERSION}</td>
												<td class="center">${var.ORG_NAME}</td>
												<td class="center">
													<c:if test="${QX.check == 1 }">
														<i class="iconfont icon-sousuo green1 marginLR-10" title="审核" onclick="goCheck('${var.ID}','${var.USER_SYS_ID}');" ></i>
													</c:if>
												</td>
											</tr>
										</c:forEach>
...
function goCheck(ID,USER_SYS_ID){
	 ...
	 diag.Drag=true;
	 diag.Title ="个人信息审核";
	 diag.URL = '<%=basePath%>ServerUtil/goAppCheck.do?ID='+ID+'&USER_SYS_ID='+USER_SYS_ID;
	 diag.Width = 500;
	 diag.Height = 650;
