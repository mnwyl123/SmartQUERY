			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="paramManage/paramlist.do?menuId=${pd.menuId}" method="post" name="Form" id="Form">
								...
								<td class="align-left width-70">
									<c:if test="${QX.query == 1 }">
										<div class="float-l margin2-5">
											<span>参数名称:</span>
											<input type="text" style="width:180px" name="PARAM_NAME" id="PARAM_NAME" placeholder="参数名称" />
										</div>
									...
									<c:if test="${QX.query == 1 }">
										<c:forEach items="${paramList}" var="var" varStatus="vs">	
											<tr>
												...
												<td class="center">${var.PARAM_VALUE }</td>
												<td class="center">${var.PARAM_FORMAT }</td>
												<td class="center">
													<a  href="javascript:edit('${var.PARAM}');">
														<i class="iconfont icon-bianji red1 " title="修改" ></i>
													</a>
													&nbsp;&nbsp;&nbsp;
													<c:if test="${var.PARAM != 'noWorkTime' }">
														<a  href="javascript:del('${var.PARAM}');">
...
<script type="text/javascript">
	...
	function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>paramManage/goSaveP.do';
		 diag.Width = 500;
		 diag.Height = 240;
	...
	function edit(PARAM){
		 ...
		 diag.Drag=true;
		 diag.Title ="资料";
		 diag.URL = "<%=basePath%>paramManage/goUpdateP.do?PARAM="+PARAM;
		 diag.Width = 469;
		 diag.Height = 240;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	function del(PARAM){
		 ...
		 diag.Drag=true;
		 diag.Title ="资料";
		 diag.URL = "<%=basePath%>paramManage/deleteP.do?PARAM="+PARAM;
		 diag.Width = 469;
		 diag.Height = 240;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
