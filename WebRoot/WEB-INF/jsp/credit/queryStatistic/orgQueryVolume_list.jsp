...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">	
						<!-- 检索  -->
						<form action="OrgQueryVolume/list.do?menuId=45" method="post" name="Form" id="Form">
								...
								<c:if test="${QX.query == 1 }">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center' hidden >${var.PRO_ID}</td>
											...
											<td class="center">
												...
												<c:if test="${QX.edit == 1 }">
													<a>
													<i class="menu-icon fa fa-book icon-shape7se" title="查看详情" onclick="showDetails('${var.APPLICATION_TIME}','${var.product_id}')"></i>
												   	</a>
											   	</c:if>
												</div> 	
											</td>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			/* $('#simple-table tr').each(function() {
			    $(this).find('td:eq(4)').each(function(){
						if($(this).text()){
							totalCom+=parseInt($(this).text());
						}
					});
				$(this).find('td:eq(5)').each(function(){
						if($(this).text()){
							totalPer+=parseInt($(this).text());
						}
				});
		...
		//查看详情
		function showDetails(APPLICATION_TIME,PRO_ID){
			APPLICATION_TIME=APPLICATION_TIME.substring(0,10);
			top.jzts();
			window.location.href='<%=basePath%>OrgQueryVolume/goDetail.do?APPLICATION_TIME='+APPLICATION_TIME+'&PRO_ID='+PRO_ID;
	    }
	</script>
