			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="HistoryAppRe/listHistoryAppRe.do?menuId=${pd.menuId}" method="post" name="Form" id="Form">
							...
							<tr>
								<c:if test="${QX.query == 1 }">
									<td class="align-left">
										...
										<div class="float-l margin2-5">
											<span>&nbsp;客户名称:</span>
											<input type="text" style="width:180px" name="CLIENT_NAME" id="CLIENT_NAME" value="${pd.CLIENT_NAME}" placeholder="请输入客户名称" />
										</div>
										<div class="float-l margin2-5">	
										    <span>&nbsp;证件类型:<span>
										    <c:if test="${pd.CLIENT_TYPE == 2 }">
												<select  dict-name="CRED_TYPE_COM" default-value="${pd.TYPE}" name="TYPE" id="TYPE" style="width: 120px;">  
												   <option value="">-请选择-</option>
												</select>
											</c:if>
											<c:if test="${pd.CLIENT_TYPE == 1  }">
												<select style="width:180px" dict-name="CRED_TYPE_PER" default-value="${pd.TYPE}" name="TYPE" id="TYPE">
													<option value="">请选择证件类型</option>
												</select>
											</c:if>
											<c:if test="${pd.CLIENT_TYPE != 2 && pd.CLIENT_TYPE != 1}">
												<select default-value="${pd.TYPE}" name="TYPE" id="TYPE" style="width: 120px;">  
												   <option value="">-请选择-</option>
												</select>
											</c:if>
										</div>
										<div class="float-l margin2-5">
											<span>&nbsp;证件号码:</span>
											<input type="text"style="width:180px" name="CCODE" id="CCODE" value="${pd.CCODE}" placeholder="请输入身份证号码/中证码" />
										</div>
							...
							<thead>
								<tr>
									...
									<th class="center">客户名称</th>
									 <c:if test="${pd.CLIENT_TYPE == 2 }">
										<th class="center" dict-name="CRED_TYPE_COM">证件类型</th>
									</c:if>
									 <c:if test="${pd.CLIENT_TYPE == 1 }">
									...
									<th class="center" dict-name="checkStatus">状态</th>
									<th class="center">申请机构</th>
									<th class="center" dict-name="DeptCode">申请部门</th>
											...
											<tr>
												...
												<td class="center">
													<c:if test="${QX.check == 1 }">
														<i class="iconfont icon-duihao red1 marginLR-10" title="详细信息" onclick="goCheck('${var.ID}','${var.USER_SYS_ID}','${var.CLIENT_TYPE}');" ></i>
													</c:if>
												</td>
											</tr>
										</c:forEach>
...
//审核
function goCheck(ID,USER_SYS_ID,CLIENT_TYPE){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="审核历史详情";
	 diag.URL = '<%=basePath%>HistoryAppRe/getCheckInfo.do?ID='+ID+'&USER_SYS_ID='+USER_SYS_ID+'&CLIENT_TYPE='+CLIENT_TYPE;
	 diag.Width = 500;
	 diag.Height = 650;
