					...
					<div class="row">
						...
						<table class="margin-5 width-100">
							<tr>
								<td class="align-left width-70">
									...
									<div class="float-l margin2-5">
										<select class="chosen-select form-control" name="name" id="id" data-placeholder="请选择" style="vertical-align:top;width: 80px;">
											<option value=""></option>
								...
								<c:when test="${r"${not empty varList}"}">
									...
									<c:forEach items="${r"${varList}"}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${r"${vs.index+1}"}</td>
										<#list fieldList as var>
											<td class='center'>${r"${var."}${var[0]}${r"}"}</td>
										</#list>
											<td class="center" >
												<c:if test="${r"${QX.edit != 1 && QX.delete != 1 }"}">
												<span class="label label-large label-grey arrowed-in-right arrowed-in"><i class="ace-icon fa fa-lock" title="无权限"></i></span>
												</c:if>
												<div class="hidden-sm hidden-xs action-buttons">
													<c:if test="${r"${QX.edit == 1 }"}">
													<a  title="编辑" onclick="edit('');"><!-- 此处的edit传入值可根据实际情况设置为表主键-->
														<i class="iconfont icon-bianji red1" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${r"${QX.delete == 1 }"}">
													<a  onclick="del('');"><!-- 此处的del传入值可根据实际情况设置为表主键-->
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${r"${QX.edit == 1 }"}">
															<li>
																<a style="cursor:pointer;" onclick="edit('');" class="tooltip-success" data-rel="tooltip" title="修改"><!-- 此处的edit传入值可根据实际情况设置为表主键-->
																	<span class="green">
															...
															<c:if test="${r"${QX.delete == 1 }"}">
															<li>
																<a style="cursor:pointer;" onclick="del('');" class="tooltip-error" data-rel="tooltip" title="删除"><!-- 此处的del传入值可根据实际情况设置为表主键-->
																	<span class="red">
						...
						<table class="width-100">
							<tr>
								...
								<td class="align-top center"><div class="pagination">${r"${page.pageStr}"}</div></td>
							</tr>
						</table>
						</div>
						</form>
	...
	<script type="text/javascript">
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>${objectNameLower}/delete.do?${objectNameUpper}_ID="+Id+"&tm="+new Date().getTime();//此处根据实际情况传入为主键
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>${objectNameLower}/goEdit.do?${objectNameUpper}_ID='+Id;//此处根据实际情况传入为主键
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
